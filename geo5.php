...
// Every request must have a user
$user = getParam('user');
...
if (!isNullOrEmptyString($lat)) {
	...
	if (!writePostion($user, getParam('group'), $lat, $lon, getParam('bearing'), getParam('speed'), getParam('altitude'), getParam('accuracy'), getParam('time'), getParam('storeTrack'))) {
...
if (!isNullOrEmptyString($positions)) {
	if (!writePostions($user, getParam('group'), $positions, getParam('storeTrack'))) {
...
if (!isNullOrEmptyString($getLastPostionsAndTracks)) {
	$postions = getLastPostionsAndTracksIndividually($getLastPostionsAndTracks, getParam('tracks'));
